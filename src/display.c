...
void init_spi_lcd_sd(){
    ...
    gpio_set_function(DISPLAY_nRESET, GPIO_FUNC_SIO);

    gpio_set_dir(DISPLAY_DC, GPIO_OUT);
    gpio_set_dir(DISPLAY_nRESET, GPIO_OUT);

    gpio_put(DISPLAY_DC, 0); // DC low
    gpio_put(DISPLAY_nRESET, 1); // nRESET high
    ...
    gpio_set_dir(ENC_A, GPIO_FUNC_SIO);
    gpio_set_dir(ENC_B, GPIO_FUNC_SIO);
    gpio_set_dir(ENC_SW, GPIO_FUNC_SIO);
}
