...
static void eq7_init(ep7_t* e, float sr) {
    ...
    float freqs[7] = {50.f, 250.f, 550.f, 1000.f, 2000.f, 4000.f, 10000.f}; //band Centers
    for (int i=0; i<7; ++i) {
...
static void updater(band_t* b, band_type_t type, float sr, float f0,float Q, float gaindB) {
    if(gaindB > 6) gaindB = 6;
    if (gaindB < -6) gaindB = -6; 
    switch (type) {
        case EQ_PEAK: b->target = peak_coef(sr, f0, Q, gainDB); break;
        case EQ_LOSHELF: b->target = lowshelf_coef(sr, f0, 1.0, gainDB); break;
        case EQ_HISHELF: b->target = highshelf_coef(sr, f0, 1.0, gainDB); break;
    }
}